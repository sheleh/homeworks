...
class TVController:
    ...
    def turn_channel(self,number):
        ...
        try:
            channel = self.channels[self.number-1]
            print(channel)
        except IndexError:
            print(f'channel #{self.number} is not in channel list')
    ...
    def is_exist(self, ch):
        ...
        if type(self.ch) is str:
            for value in self.channels:
                if value.lower() == self.ch.lower():
                    res = True
        elif type(self.ch) is int:
            if self.ch in range(len(self.channels)+1):
                res = True
        if res:
            print('Yes')
        else:
